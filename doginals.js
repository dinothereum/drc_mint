...
async function walletSync() {
    ...
    while (true) {
        try {
            ...
            console.log('syn failed 2', e?.message)
            if (e.message === 'Request failed with status code 502') {
                console.warn('retrying, cannot connect websocket ...')
...
async function broadcast_back(tx, retry) {
    ...
    while (true) {
        try {
            ...
            console.log("ERROR sendrawtransaction = " + e.message)
            if (e.message === 'Socket connection timeout' || e.message === 'getaddrinfo ENOTFOUND doge.getblock.io') {
                console.warn('retrying, cannot connect websocket ...')
...
async function broadcast(tx, retry) {
    ...
    while (true) {
        try {
           const resp =  await axios.post('https://api.blockchair.com/dogecoin/push/transaction', body, options)
            if (resp.status != 200) {
                let json;
                try {
